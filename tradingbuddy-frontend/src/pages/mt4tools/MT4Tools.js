...
const MT4Tools = () => {
  ...
  const Chart = () => {
    ...
    return (
      <div>
        {
          elementsChart.length ? <CanvasJSChart options={options} /> :
            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'cetner', justifyContent: 'center' }}>
              <span style={{ fontSize: '20px', color: 'gray' }}>No data</span>
            </div>
  ...
  const [isWidget4OnLoading, setIsWidget4OnLoading] = useState(true);
  // Widget 5
  const [widget5Data, setWidget5Data] = useState([]);
  const [isWidget5OnLoading, setIsWidget5OnLoading] = useState(true);

  // Widget 6
  const [widget6Data, setWidget6Data] = useState([]);
  const [isWidget6OnLoading, setIsWidget6OnLoading] = useState(true);

  // Widget 7
  const [widget7Data, setWidget7Data] = useState([]);
  const [isWidget7OnLoading, setIsWidget7OnLoading] = useState(true);

  // Widget 8
  const [widget8Data, setWidget8Data] = useState([]);
  const [isWidget8OnLoading, setIsWidget8OnLoading] = useState(true);
  ...
  const get5WidgetsData = async () => {
    await getWidget4Data();
      await getWidget5Data();
      await getWidget6Data();
      await getWidget7Data();
      await getWidget8Data();
  }

  const getWidget4Data = async () => {
    try {
  ...
  const getWidget7Data = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/mttools/getwidget7data`);
      //Widget 7
                  ...
                  <div className="combo-group">
                    ...
                    <div style={{ minWidth: '120px', color: '#bdbdbd' }}>
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => handleChange('OPEN_DATE', new Date(date.setHours(0, 0, 0, 0)))}
                      />
                    </div>
                    <div style={{ minWidth: '120px' }}>
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => handleChange('CLOSED_DATE', new Date(date.setHours(0, 0, 0, 0)))}
                      />
                    </div>
